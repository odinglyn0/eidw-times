...
import { utcToZonedTime } from "https://esm.sh/date-fns-tz@2.0.0?deps=date-fns@3.6.0"; // Explicitly define date-fns dependency
...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      console.error("Supabase query error in get-departure-data:", error);
    ...
    console.log(`Data from departures for T${terminalId} via Edge Function:`, data);
    ...
    console.error("Edge Function error in get-departure-data:", error);
    return new Response(JSON.stringify({ error: error.message }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
